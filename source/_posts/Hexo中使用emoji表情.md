...
date: 2022-05-22 20:43:34
tags: 
  - 博客美化
  - hexo
categories: 
  - hexo
description: Hexo开启欢乐的emoji之旅
index_img: https://dognew-1312375098.cos.ap-nanjing.myqcloud.com/%E6%9D%82%2F45%20-%20pWREJfT.jpg
---

# Hexo中使用emoji表情

Hexo默认的markdown渲染引擎不支持将Github emoji渲染到静态的html页面中，我们换一个支持emoji的引擎，再增加一个emoji插件即可。
...
完成插件安装后还需要修改Hexo站点配置文件`_config.yml`(不是主题配置)
...
这里需要注意`render:`下的`html:`配置项，它的作用是控制`Markdown`渲染引擎是否转移文档中出现的`html`标签，默认为`false`，这里要设置为`true`，否则会导致`<!--more-->`渲染失败。
...
`plugins:`中的最后一项`- markdown-it-emoji`是手动添加的，官方Github Wiki中给出的配置不包含这一项，其他配置参照的Github Wiki中的默认配置。

## 使用方法

输入对应的emoji编码

例如：输入笑脸对应的emoji编码`:smile:`就可以得到:smile:
...
转载来源：[Hexo中使用emoji表情 - 银河小徐 (hasaik.com)](https://hasaik.com/posts/9b280ea3.html)
